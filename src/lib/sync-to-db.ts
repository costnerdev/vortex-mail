...
export async function syncEmailToDatabase(emails: EmailMessage[], accountId: string) {
    console.log('attemtping to sync emails to the database', emails.length);
    ...
    try {
        ...
        // Change from concurrent to sequential processing
...
async function upsertEmail(email: EmailMessage, accountId: string, index: number) {
    ...
    try {
        ...
        for(const address of addressesToUpsert.values())  {
            const upsertedAddress = await upsertEmailAddress(address, accountId); // Sync email address to db prod
        ...
        const fromAddress = addressMap.get(email.from.address); // get the from address object
        if (!fromAddress) {
        ...
        const thread = await db.thread.upsert({
            ...
            update: {
                ...
                participantIds: [...new Set([
                    fromAddress.id,
                    ...toAddresses.map(e => e!.id),
                    ...ccAddresses.map(e => e!.id),
                    ...bccAddresses.map(e => e!.id),
                ])],
            },
            create: {
                id: email.threadId,
...
async function upsertEmailAddress(address: EmailAddress, accountId: string) {
    try {
        ...
        console.log('Error while upserting email address', error);
        return null;
    }
}